...
- **`lib/config.ts`** — the source of truth for domain options: `COACHES` (roster, type-ahead picker), `OUTCOMES` (the four gated approval/denial types), `WRONG_CLASS` + `LOG_OUTCOMES` (`OUTCOMES` plus the wrong-class note — what a coach may actually log), and `feedbackRequired(outcome)`. **This module is imported by both client and server**, so it must stay free of `"use server"` and server-only imports. Editing the roster or entry types happens here.
...
- **`verdict` column stores the entry type** (one of `LOG_OUTCOMES`), not a yes/no. `toEval()` maps it to `outcome` for the rest of the code.
...
- **Wrong-class is not a gated decision.** `WRONG_CLASS` stays out of `OUTCOMES` on purpose: it never counts toward standings, disagreements, or "evaluation completed" votes, and it is never offered as a final verdict. It gets its own amber block in the log form (attended vs. correct level) — anything that enumerates *approval* calls must keep using `OUTCOMES`, not `LOG_OUTCOMES`.
